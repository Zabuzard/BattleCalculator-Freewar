...
// CSS class used on elements generated by this script.
 ...
 *
 * The important difference from the old implementation is that we
 * no longer put "processed" state on the NPC name element.
 *
 * Instead, the .fastattack element itself contains a
 * .battlecalculator-result element when this script has modified it.
 *
 * If Freewar replaces the fastattack element, our result element is
 * gone as well, so the NPC will automatically be processed again.
 *
 * @param cellElement The element that might contain NPC data
 *
 * @return True if the player can defeat the NPC, false otherwise
 */
...
function processElement(cellElement) {
	...
	var $npcNameElement = $cell.find('b');
	var $npcFastAttackElement = $cell.find('.fastattack');
	...
	/*
	 * Do not use a class on the NPC name as the processed state.
	 *
	 * Instead, check whether our generated result is currently present
	 * inside the fastattack element.
	 */
	...
	/*
	 * Ignore list for auto attack
	 */

	if (isIgnoredNpc(npcName) && npcAutoAttack) {
		// Nothing because this is your ignore list for auto attack
		return false;
	}
	...
	/*
	 * Critical life loss gets orange.
	 * Otherwise the result is green.
	 *
	 * The old code set orange first and then immediately overwrote
	 * it with green. This fixes that.
	 */
	...
	/*
	 * NPC auto attack if enabled
	 */

	if (npcAutoAttack) {
		const randomWait = Math.floor(
...
function extractNpcStats(cellElement) {
	var text = cellElement.textContent || '';
	var lifeMatch = text.match(/LP\s*:\s*[\d.]+\s*\/\s*([\d.]+)/i);
	var strengthMatch = text.match(/Angriffsstärke\s*:\s*([\d.]+)/i);
...
/*
 * Adds a result generated by BattleCalculator.
 *
 * This is the main state marker for the script.
 *
 * Instead of adding raw text directly to .fastattack, we add a
 * dedicated child element. This means we can reliably determine
 * whether the current .fastattack has already been processed.
 *
 * @param $element The fastattack element
 * @param text The text to append
 * @param cssClass Optional additional result class
 *
 * @return The generated result element
 */

function addBattleCalculatorResult($element, text, cssClass) {
	var $result = $('<span>')
		.addClass(battleCalculatorResultClass)
		.text(text);
...
/*
 * Checks whether an NPC is on the ignore list.
 */

function isIgnoredNpc(npcName) {
...
function computeOutcome(playerLife, playerStrength, playerDefense, npcName, npcStats) {
...
/*
 * Returns whether the given NPC is a critical special NPC.
 *
 * @param npcName The name of the NPC
 *
...
/*
 * Gets the value of the given player stat.
 * The value is saved via webstorage or as cookie.
 *
 * @param statName The name of the player stat to get
 * @returns The value of the stat, greater equals 0 or -1 if invalid/not found
 */
...
function getStatValue(statName) {
	...
	if (isSupportingWebStorage()) {
		...
		value = sessionStorage.getItem(
			'freewarBattleCalculatorStat' + statName
		);
	} else {
...
/*
 * Extracts the value of the given player stat.
 * The value is saved via webstorage or as cookie.
 *
 * @param statName The name of the player stat to get
 * @param fallbackValue The value to use if the extracted value is nonexistent or invalid
 *
 * @returns The extracted value of the stat, greater equals 0 or the fallback value
 */
